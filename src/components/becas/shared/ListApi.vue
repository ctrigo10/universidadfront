    ...
    >
      <template v-slot:[`item.archivo`]="{ item }">
        <v-tooltip
          top
          v-if="
            acciones.archivo && item.seleccionar_becados != true && item.archivo
          "
        >
          <template v-slot:activator="{ on, attrs }">
            <a
              :href="host + item.pathFile"
              target="_blank"
              v-bind="attrs"
              v-on="on"
      ...
      <template v-slot:[`item.actions`]="{ item }">
        <v-tooltip
          top
          v-if="acciones.ingresar && item.seleccionar_becados != true"
        >
          <template v-slot:activator="{ on, attrs }">
        ...
        <v-tooltip
          top
          v-if="acciones.cuestionario && item.seleccionar_becados != true"
        >
          <template v-slot:activator="{ on, attrs }">
        ...
        <v-tooltip
          top
          v-if="acciones.solicitarBeca && item.seleccionar_becados != true"
        >
          <template v-slot:activator="{ on, attrs }">
        ...
        <v-tooltip
          top
          v-if="acciones.editar && item.seleccionar_becados != true"
        >
          <template v-slot:activator="{ on, attrs }">
        ...
        <v-tooltip
          top
          v-if="acciones.eliminar && item.seleccionar_becados != true"
        >
          <template v-slot:activator="{ on, attrs }">
        ...
        <v-tooltip
          top
          v-if="acciones.finalizar && item.seleccionar_becados != true"
        >
          <template v-slot:activator="{ on, attrs }">
...
export default {
  name: "becas-list",
  props: ["headers", "items", "acciones", "sortby","itemsTotal","loading"],
  components: {},
  ...
  methods: {
    ...
    deleteItem(item) {
      this.$vuetifyConfirmDialog
        ...
        .then((state) => {
          if (state == true) this.$emit("deleteItem", item);
        });
    },
